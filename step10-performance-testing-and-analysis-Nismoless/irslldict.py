...
class IRSystemLL(IRSystemAbstract):
    def __init__(self):
        self._dict_items = LLDict()

    def search_by_id(self, song_id): # Rob Eusanio (given)
        """Return an item object matching the given unique identifier, or None if not found."""
        if song_id in self._dict_items:  # searching through the dictionary using the magic method
            return self._dict_items[song_id]
        return None
